import pandas as pd
from freq_cleaner import predict
import numpy as np
...
def find_intersection(row):
    ...
    noun_techs_lower = set([tech.lower() for tech in row['clean_techs']])
...
def evaluate_model(test_data_path):
    ...
    test_inference['FN'] = test_inference.apply(find_unique_techs, args=('all_techs', 'clean_techs'), axis=1)
    test_inference['FN_N'] = test_inference['FN'].apply(count_techs)

    test_inference['FP'] = test_inference.apply(find_unique_techs, args=('clean_techs', 'all_techs'), axis=1)
    ...
    test_inference.to_csv('frequency_denoiser/test_inference_evaluation.csv')
